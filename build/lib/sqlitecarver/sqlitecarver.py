...
__usage__ = f"Parse deleted records from an SQLite file into a TSV File or text file \n\
Examples:\n\
{os.path.basename(__file__)} -f /home/sanforensics/smsmms.db -o report.tsv\n\
{os.path.basename(__file__)} -f /home/sanforensics/smssms.db -r -o report.txt \n"
...
def parse_db_file(args, output):
    ...
    # filesize = len(f.read())
    # Cheeky suggestion ... so it doesn't read the whole file unecessarily
    ...
    while offset < filesize:
        ...
        # So just print strings from all non flag = 13 pages.
            ...
            f.seek(
                -1, 1
            )  # since flag is not 13, we don't care what the flag is, lets go back and get that byte in case it's string material
            pagestring = f.read(pagesize)  # we've already read the flag byte
